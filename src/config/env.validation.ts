...
export const envSchema = z.object({
  ...
  JWT_ACCESS_EXPIRY: z.string().regex(/^\d+[smhd]$/, 'JWT_ACCESS_EXPIRY must be in format: 60m, 1h, etc.').default('60m'),
  JWT_REFRESH_EXPIRY: z.string().regex(/^\d+[smhd]$/, 'JWT_REFRESH_EXPIRY must be in format: 7d, 30d, etc.').default('30d'),
